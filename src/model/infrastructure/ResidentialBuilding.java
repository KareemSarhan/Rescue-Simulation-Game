...
public class ResidentialBuilding implements Rescuable, Simulatable 
{
	...
	public boolean respondable=true;
	...
	public String displayDisaster() {
		if(this.getDisaster() != null )
		{
			return this.getDisaster().toString();
		}
		else 
			return "None";
	}
	public String displayOccupants() {
		String r="";
		for(int i=0;i<this.getOccupants().size();i++) {
			r=r+"\n"+this.getOccupants().get(i).display()+"\n"+"-----------------------------------";
		}
		return r;
	}
	...
	public String display() {
		return "Location :- ("+this.getLocation().getX()+","+this.getLocation().getY()+") \n"+
				...
				"Occupants Number :- "+this.getOccupants().size()+"\n"+
				"Disaster :- "+this.displayDisaster()+"\n"+
